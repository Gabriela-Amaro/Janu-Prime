...
class AdministradorRegistrationSerializerTest(TestCase):
  ...
  def test_administrador_registration_serializer_valid_data(self):
    ...
    self.assertEqual(serializer.validated_data["estabelecimento"].id, data["estabelecimento"])
  ...
  def test_administrador_registration_serializer_invalid_data(self):
    ...
    self.assertEqual(serializer.errors["email"][0].title(), "Este Campo Não Pode Estar Em Branco.")
    
    self.assertEqual(serializer.errors["password2"][0].code, "required")
    self.assertEqual(serializer.errors["password2"][0].title(), "Este Campo É Obrigatório.")

    self.assertEqual(serializer.errors["nome"][0].code, "required")
    self.assertEqual(serializer.errors["nome"][0].title(), "Este Campo É Obrigatório.")

    self.assertEqual(serializer.errors["cpf"][0].code, "unique")
    self.assertEqual(serializer.errors["cpf"][0].title(), "Administrador Com Este Cpf Já Existe.")

    self.assertEqual(serializer.errors["estabelecimento"][0].code, "does_not_exist")
    self.assertEqual(serializer.errors["estabelecimento"][0].title(), 'Pk Inválido "999999" - Objeto Não Existe.')
